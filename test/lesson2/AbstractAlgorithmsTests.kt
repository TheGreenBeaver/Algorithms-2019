...
Но вреден север для меня
    ...
    fun baldaSearcher(baldaSearcher: (String, Set<String>) -> Set<String>) {
        ...
        // My tests
        assertEquals(
            setOf("ЛЕЖАЧИЙ", "КРОЛИЧИЙ", "БРОНТОЗАВР", "ВЫМЫВАНИЕ", "КУЗОВ", "СПУСК"),
            baldaSearcher(
                "input/balda_in4.txt",
                setOf(
                    "ЛЕЖАЧИЙ", "КРОЛИЧИЙ", "БРОНТОЗАВР", "БИГЛЬ", "КОНКАТЕНАЦИЯ", "КОРИНФ", "ФИВЫ", "УКСУС",
                    "ВЫМЫВАНИЕ", "КУЗОВ", "СПУСК", "АБРАКАДАБРА"
                )
            )
        )
    }
